@subjectLookupV2.1 @LookupAPI-V2.1 @Lookup
Feature: ||LookUps|| SubjectLookup Endpoint || API V2.1||
  ...
  @subjectLookupV2.1 @API-V2.1 @Sanity
  Scenario Outline: 1. Call subjectLookup webservice to get the subject list and Verify  lookup data and status
    ...
    Examples: 
      | parameter                   | Lookup        |
      | /api/2.1/data/subjectLookup | subjectLookup |

  @SubjectLookupswithQueryParameterV2.1 @WEBAPIAutomation @API-V2.1
  Scenario Outline: 2. Call webservice to get the LooksUPlist with query parameter and Verify lookup data and status
    ...
    Examples: 
      ...
      | /api/2.1/data/subjectLookup | q          | Fuel, energy and natural resources | pageIndex  |      1 |
